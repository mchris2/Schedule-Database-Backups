<#
.SYNOPSIS
    Schedules a one-off SQL Server database back-up job using SQL Server Agent.
...
.DESCRIPTION
    ...
    It checks:
      ...
      - That the SQL Agent job name does not already exist
    The script creates the job, adds back-up steps for each database using native T-SQL commands, and attaches a one-off schedule.
...
.NOTES
    - Requires dbatools PowerShell module on the client machine only.
    ...
    - The job is scheduled as a one-off and will be removed after it completes.
#>
...
# Create the SQL Agent Job and steps using T-SQL commands instead of PowerShell
try {
    $null = New-DbaAgentJob -SqlInstance $SqlInstance -Job $JobName -Description $Description -Force
    foreach ($db in $Databases) {
        $stepName = "Backup $db"
        
        # Use proper T-SQL timestamp format that will be evaluated at runtime on SQL Server
        $command = @"
DECLARE @timestamp NVARCHAR(50) = CONVERT(NVARCHAR(8), GETDATE(), 112) + '_' + 
                               REPLACE(CONVERT(NVARCHAR(8), GETDATE(), 108), ':', '');
DECLARE @backupPath NVARCHAR(260) = N'$FilePath\$db-' + @timestamp + N'.bak';

BACKUP DATABASE [$db] TO DISK = @backupPath 
WITH COPY_ONLY, STATS = 10, COMPRESSION, CHECKSUM;
"@
        $null = New-DbaAgentJobStep -SqlInstance $SqlInstance -Job $JobName -StepName $stepName -Subsystem TransactSql -Command $command
    }
} catch {
...
# Add a final job step to export job history and remove the job itself (using T-SQL)
try {
    $finalStepName = "Export Job History and Remove Job"
    # T-SQL version that doesn't require dbatools
    # Replace the current finalStepCommand with this simpler version
$finalStepCommand = @"
-- Create a log file at the backup location with job history
DECLARE @JobName NVARCHAR(128) = N'$JobName';
DECLARE @FilePath NVARCHAR(4000) = N'$FilePath';
DECLARE @DateStamp NVARCHAR(50) = CONVERT(NVARCHAR(8), GETDATE(), 112) + '_' + 
                                  REPLACE(CONVERT(NVARCHAR(8), GETDATE(), 108), ':', '');
DECLARE @LogFile NVARCHAR(4000) = @FilePath + N'\' + @JobName + N'_BackupLog_' + @DateStamp + '.txt';
DECLARE @LogContent NVARCHAR(MAX);
DECLARE @Cmd NVARCHAR(4000);

-- Build log content with job information
SET @LogContent = '============ SQL SERVER BACKUP JOB RESULTS ============' + CHAR(13) + CHAR(10);
SET @LogContent = @LogContent + 'Job Name: ' + @JobName + CHAR(13) + CHAR(10);
SET @LogContent = @LogContent + 'Completed at: ' + CONVERT(VARCHAR(20), GETDATE(), 120) + CHAR(13) + CHAR(10);
SET @LogContent = @LogContent + 'Backup Location: ' + @FilePath + CHAR(13) + CHAR(10);
SET @LogContent = @LogContent + CHAR(13) + CHAR(10);
SET @LogContent = @LogContent + '---------------- BACKUP RESULTS ----------------' + CHAR(13) + CHAR(10);
...
WHERE j.name = @JobName AND h.step_id > 0 -- Exclude the job outcome (step_id = 0)
ORDER BY h.run_date, h.run_time, h.step_id;

-- Add results for each step to the log content
DECLARE @StepId INT, @StepName NVARCHAR(128), @RunStatus INT, @Message NVARCHAR(4000);
DECLARE @FormattedDate NVARCHAR(30);
DECLARE @TotalSteps INT, @SuccessSteps INT = 0, @FailedSteps INT = 0;

SELECT @TotalSteps = COUNT(*) FROM @StepResults;

DECLARE step_cursor CURSOR FOR 
SELECT StepId, StepName, RunStatus, Message FROM @StepResults;

OPEN step_cursor;
FETCH NEXT FROM step_cursor INTO @StepId, @StepName, @RunStatus, @Message;
...
BEGIN
    ...
    IF @RunStatus = 1 
    BEGIN
        SET @LogContent = @LogContent + 'Step ' + CAST(@StepId AS NVARCHAR(10)) + ': [SUCCESS] ' + @StepName + CHAR(13) + CHAR(10);
        SET @SuccessSteps = @SuccessSteps + 1;
    END
    ELSE
    BEGIN
        SET @LogContent = @LogContent + 'Step ' + CAST(@StepId AS NVARCHAR(10)) + ': [FAILED] ' + @StepName + CHAR(13) + CHAR(10);
        SET @LogContent = @LogContent + '   Error: ' + @Message + CHAR(13) + CHAR(10);
...
-- Add summary
SET @LogContent = @LogContent + CHAR(13) + CHAR(10);
SET @LogContent = @LogContent + '---------------- SUMMARY ----------------' + CHAR(13) + CHAR(10);
SET @LogContent = @LogContent + 'Total Steps: ' + CAST(@TotalSteps AS NVARCHAR(10)) + CHAR(13) + CHAR(10);
SET @LogContent = @LogContent + 'Successful: ' + CAST(@SuccessSteps AS NVARCHAR(10)) + CHAR(13) + CHAR(10);
SET @LogContent = @LogContent + 'Failed: ' + CAST(@FailedSteps AS NVARCHAR(10)) + CHAR(13) + CHAR(10);
SET @LogContent = @LogContent + CHAR(13) + CHAR(10);
SET @LogContent = @LogContent + '===========================================' + CHAR(13) + CHAR(10);

-- Write the log to a file using xp_cmdshell
SET @Cmd = 'echo ' + REPLACE(@LogContent, CHAR(13) + CHAR(10), '^' + CHAR(13) + CHAR(10) + 'echo ') + ' > "' + @LogFile + '"';

-- Check if xp_cmdshell is enabled
DECLARE @ConfigValue INT;
EXEC master.dbo.sp_configure 'show advanced options', 1;
RECONFIGURE;
EXEC master.dbo.sp_configure 'xp_cmdshell', @ConfigValue OUTPUT;

IF @ConfigValue = 0
BEGIN
    -- If xp_cmdshell is disabled, write a message to job history
    PRINT 'Cannot write log file: xp_cmdshell is disabled on this server.';
    PRINT 'Job history will only be available in SQL Server Agent Job History.';
    PRINT 'Job summary: Total=' + CAST(@TotalSteps AS NVARCHAR(10)) + ', Success=' + CAST(@SuccessSteps AS NVARCHAR(10)) + ', Failed=' + CAST(@FailedSteps AS NVARCHAR(10));
END
ELSE
BEGIN
    -- Execute the command to write the log file
    EXEC master.dbo.xp_cmdshell @Cmd;
    PRINT 'Job log file written to: ' + @LogFile;
END

-- Wait to ensure any in-progress activity completes
WAITFOR DELAY '00:00:05';

-- Delete the job
EXEC msdb.dbo.sp_delete_job @job_name = @JobName;
"@
    $null = New-DbaAgentJobStep -SqlInstance $SqlInstance -Job $JobName -StepName $finalStepName -Subsystem TransactSql -Command $finalStepCommand
    
    # Configure job step flow
    try {
...
foreach ($step in $jobInfo) {
    if ($step.step_name -eq "Export Job History and Remove Job") {
        ...
        Write-Host ("    Action: Job history saved to msdb.dbo.JobHistory_$JobName")
        Write-Host ("    Action: Job will be removed after completion (sp_delete_job)")
    } else {
        Write-Host ("  Step {0}: {1}" -f $step.step_id, $step.step_name)
        # Format the T-SQL command to make it more readable
        $formattedCommand = $step.command -replace "WITH ", "`nWITH "
        Write-Host ("    Command: {0}" -f $formattedCommand)
    }
}
Write-Host ""
Write-Host "Schedule:"
